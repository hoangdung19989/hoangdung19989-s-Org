...
import type { MockExamSubject, TestGrade } from '../types';
import { HomeIcon, ChevronRightIcon } from './icons';

interface MockExamGradeSelectionViewProps {
...
const MockExamGradeSelectionView: React.FC<MockExamGradeSelectionViewProps> = ({ 
    subject, 
    grades, 
    onSelectGrade, 
    onBackToSubjects, 
                ...
                <div className="flex bg-white rounded-xl shadow-md border border-slate-200 overflow-hidden">
                    {grades.map((grade, index) => (
                        <button
                            ...
                            className={`px-10 py-4 text-lg font-semibold text-slate-700 hover:bg-amber-50 transition-colors focus:outline-none focus:bg-amber-100 focus:ring-2 focus:ring-amber-500 z-10
...
export default MockExamGradeSelectionView;